...
def array2hexstring(array, dtype, pad_to_nbits, prefix="0x", reverse=False):
    """
    Pack given one-dimensional NumPy array with DataType dtype into a hex
    string.
...
def unpack_innermost_dim_from_hex_string(
    ...
    """Convert a NumPy array of hex strings into a NumPy array by unpacking
...
def numpy_to_hls_code(
    ...
    """Return C++ code representation of a numpy ndarray with DataType
...
def npy_to_rtlsim_input(input_file, input_dtype, pad_to_nbits, reverse_inner=True):
    ...
    pack_innermost_dim_as_hex_string() for more info on how the
...
def py_to_packed_bytearray(
    ndarray, dtype, reverse_inner=False, reverse_endian=False, fast_mode=False
):
    """Given a numpy ndarray with DataType dtype, pack the innermost
...
def packed_bytearray_to_py(
    ...
    """Given a packed numpy uint8 ndarray, unpack it into a array of
    given DataType.
    ...
    if (
        ...
        raise Exception("packed_bytearray_to_py needs NumPy uint8 arrays")
    if packed_bytearray.ndim == 0:
        raise Exception("packed_bytearray_to_py expects at least 1D ndarray")
